...
음식 사진 분석 서비스용 텍스트 기반 실시간 통신 모듈
"""
...
# 오디오 관련 유틸리티 함수들은 음식 사진 분석 서비스에서 사용하지 않음
...
class RealtimeAPI(RealtimeEventHandler):
    def __init__(self):
        ...
        self.url = os.environ.get("OPENAI_REALTIME_URL", self.default_url)
    ...
    async def connect(self, model=None):
        try:
            ...
            self.ws = await websockets.connect(
                f"{self.url}?model={model_name}",
                additional_headers=headers,
            )
            self.log(f"Connected to {self.url} using model {model_name}")
...
class RealtimeConversation:
    ...
    default_frequency = 24000  # 오디오 샘플링 레이트 (사용하지 않지만 호환성 유지)
    ...
    def _process_audio_delta(self, event):
        """오디오 델타 처리 (음식 사진 분석 서비스에서는 사용하지 않음)"""
        item_id = event["item_id"]
        delta = event["delta"]
        item = self.item_lookup.get(item_id)
        if not item:
...
class RealtimeClient(RealtimeEventHandler):
    """음식 사진 분석을 위한 OpenAI Realtime 클라이언트"""

    def __init__(self, system_prompt: str = None, max_tokens: int = 4096):
        super().__init__()
        self.system_prompt = system_prompt or "당신은 도움이 되는 AI 어시스턴트입니다."
        self.max_tokens = max_tokens
        self.default_session_config = {
            "modalities": ["text"],  # 텍스트 전용 모드
    ...
    async def _call_tool(self, tool):
        """MCP 도구 호출 처리 (음식 사진 분석 서비스에서는 MCP 기능 미사용)"""
        try:
            ...
            logger.info(f"MCP 함수 호출 완료: {tool_name}")
    ...
    async def connect(self):
        ...
        logger.info("RealtimeClient 연결 중...")

        try:
            if self.is_connected():
                logger.warning("RealtimeClient가 이미 연결되어 있습니다")
                raise Exception(
                    "이미 연결되어 있습니다. disconnect()를 먼저 호출하세요"
                )
            ...
            logger.info(f"{len(mcp_tools)}개의 MCP 도구를 가져왔습니다")

            self.session_config.update({"tools": mcp_tools})

            await self.update_session()
            logger.info("RealtimeClient 연결 완료")
            return True
        except Exception as e:
            logger.error(f"RealtimeClient 연결 오류: {str(e)}")
            logger.error(f"Traceback: {traceback.format_exc()}")
            raise e
    ...
    async def wait_for_session_created(self):
        """세션 생성 대기"""
        if not self.is_connected():
            raise Exception("연결되지 않았습니다. connect()를 먼저 호출하세요")
        while not self.session_created:
            await asyncio.sleep(0.001)
        return True

    async def disconnect(self):
        """OpenAI Realtime API 연결 해제"""
        logger.info("RealtimeClient 연결 해제 중...")
        ...
        try:
            ...
            logger.info("RealtimeClient 연결 해제 완료")
        except Exception as e:
            logger.error(f"RealtimeClient 연결 해제 오류: {str(e)}")
            logger.error(f"Traceback: {traceback.format_exc()}")
            raise e
    ...
    async def create_response(self):
        try:
            if (
                self.get_turn_detection_type() is None
                and len(self.input_audio_buffer) > 0
            ):
